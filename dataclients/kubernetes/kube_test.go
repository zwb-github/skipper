...
func TestIngressData(t *testing.T) {
	for _, ti := range []struct {
		...
		expectedRoutes: map[string]string{
			"kube_foo__baz______": "", // shuntroute, because service invalid
		},
	}, {
		...
		expectedRoutes: map[string]string{
			"kube_foo__baz__www_example_org_____bar": "", // shuntRoute: because no targetPort
		},
	}, {
		...
		expectedRoutes: map[string]string{
			"kube_foo__qux______":                    "", // shuntRoute "http://1.1.1.0:8080",
			"kube_foo__qux__www_example_org_____baz": "", // shuntRoute "http://1.1.2.0:8181",
		},
	}, {
...
func TestIngress(t *testing.T) {
	...
	t.Run("has ingresses, update some of them", func(t *testing.T) {
		...
		checkRoutes(t, r, map[string]string{
			...
			"kube_namespace1__mega__foo_example_org___test1__service1": "", //shunted because invalid backend "http://1.2.3.4:9999",
		})
	})
	...
	t.Run("has ingresses, mixed insert, update, delete", func(t *testing.T) {
		...
		api.ingresses.Items[2].Spec.Rules = api.ingresses.Items[2].Spec.Rules[:1]
		// api.endpoints.Items[0].Subsets[0].Ports[0].Name = "port2"
		// api.endpoints.Items[0].Subsets[0].Ports[0].Port = 9999
		...
		checkRoutes(t, r, map[string]string{
			"kube_namespace1__new1__new1_example_org_____service1":          "http://1.1.1.0:8080",
			"kube_namespace1__new2__new2_example_org_____service2":          "http://1.1.2.0:8181",
			"kube_namespace2__path_rule_only__www_example_org_____service3": "", // "http://2.1.3.0:4444",
		})
